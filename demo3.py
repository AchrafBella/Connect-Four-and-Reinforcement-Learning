...
if __name__ == "__main__":
    ...
    def process(df, agent, epsilon, step_size, win):
        df_ = df[agent.get_agent_name()]
        df_.at[epsilon, str(step_size)] = win
        df[agent.get_agent_name()] = df_
    ...
    for agent in agents:
        for epsilon in epsilons:
            for step_size in step_sizes:
                ...
                winning_rounds_agent1, _, utility, actions = env1.__run__(rounds=1000)
                process(data, agent, epsilon, step_size, winning_rounds_agent1)
                pass
            pass
        pass
    pass
